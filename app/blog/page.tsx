    ...
    <main className="relative min-h-screen bg-[#050508] text-[#f0f0ee]">
      ...
      <section className="relative pt-44 pb-20 px-6 overflow-hidden text-center">
        ...
        <div className="relative z-10 max-w-3xl mx-auto space-y-4">
          ...
          <h1 className="text-4xl md:text-5xl font-light leading-tight tracking-tight text-white/88 font-display">
            Le Blog Powamekka
          </h1>
          <p className="text-sm md:text-base text-white/42 max-w-xl mx-auto leading-relaxed">
      ...
      <section className="pb-40 px-6 relative z-10">
        <div className="max-w-6xl mx-auto">
          {posts.length === 0 ? (
            ...
            >
              <p className="text-white/28 text-sm">Aucun article n'a encore été publié. Revenez bientôt !</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {posts.map((post) => (
                <article key={post.slug} className="group relative">
                  ...
                  <Link 
                    href={`/blog/${post.slug}`}
                    className="block h-full p-6 rounded-2xl flex flex-col justify-between transition-all duration-300 group-hover:translate-y-[-4px]"
                  ...
                  >
                    <div className="space-y-4">
                      ...
                      {/* Title */}
                      <h3 className="text-lg font-light text-white/88 font-display group-hover:text-[#c9a96e] transition-colors duration-300 line-clamp-2">
                        {post.title}
                      </h3>

                      {/* Excerpt */}
                      <p className="text-xs text-white/42 leading-relaxed line-clamp-3">
                        {post.excerpt}
                      </p>
                    </div>
                    ...
                    <div className="pt-6 flex items-center justify-between">
                      ...
                      <span className="text-xs text-white/70 group-hover:text-[#c9a96e] transition-colors duration-300 flex items-center gap-1.5 font-medium">
                        Lire l'article
